...
Page({
  ...
  onLoad: function (option) {
     ...
     for(var i in cart_items){
          var dishsno =  cart_items[i].dishsno
          if(linyongMap.has(dishsno+'maxNum')){
             var maxNum = linyongMap.get(dishsno+'maxNum')
             linyongMap.set(dishsno+'maxNum',maxNum+cart_items[i].count);
          }else{
             linyongMap.set(dishsno+'maxNum',cart_items[i].count);
             linyongMap.set(dishsno+'choosedNum',0);  
          }
          //=================================================================
      ...
      var param = {
        ...
        totalFee:option.totalFee*100       //餐盒不参与优惠券满减
      }
  ...
  chooseTap:function(e,a,b){          
    ...
    //选 领用券 跟 品类券的逻辑
        ...
        if(items[parama].data[paramb].type.category == 6){                      //选择的券是品类券
                ...
                for(var i in items[1].data){
                    if(!items[1].data[i].active){
                        for(var j in cart_items){ 
                             if(cart_items[j].dishsno == items[1].data[i].goodsId){
            ...
            for(var i in cart_items){ 
                if(cart_items[i].dishsno==goodsId){                           //领用券对应商品
